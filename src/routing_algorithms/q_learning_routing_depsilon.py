...
class QLearningRouting_DEpsilon(BASE_routing):
    ...
    def __init__(self, drone, simulator):
        ...
        for i in range(self.NUMBER_OF_CELL):
            self.q_table[i] = np.array([0.0 for i in range(self.NUMBER_OF_ACTIONS)])
    ...
    def TD(self, reward, list_action, Q_old):
        ...
        return reward + self.GAMMA * np.max(list_action) - Q_old
    ...
    def compute_reward(self, delay, outcome, drone):
        ...
        if outcome == 1:
            ...
            reward = (
                2 * drone.buffer_length() / delay   # TODO: understand difference of self.drone and drone
    ...
    def feedback(self, drone, id_event, delay, outcome):
        ...
        if id_event in self.taken_actions:
            ...
            # print(f"Q-Table: {self.self.q_table}")
    ...
    def relay_selection(self, opt_neighbors: list, packet):
        ...
        # state = cell_index = util.TraversedCells.coord_to_cell(size_cell=self.simulator.prob_size_cell,
        ...
        epsilon = self.MIN_EPSILON + (self.MAX_EPSILON - self.MIN_EPSILON)*np.exp(-self.DECAY_RATE*cur_step)
        ##print(epsilon)
        if self.random_gen.uniform(0, 1) < epsilon:
            ...
            if self.q_table[state][0] == self.q_table[state][1]:
                ...
                action = np.where(self.q_table[state] == (np.max(self.q_table[state])))[0]
            ...
            if action == 0:
                    ...
                    if delta >= 0 and dy <= 0:
                        ...
                        if ry1 >= 0 or ry2 >= 0:
                            ...
                            * util.euclidean_distance(
                            ...
                            #distance = util.euclidean_distance(hello_pkt.cur_pos, self.drone.depot.coords)
                ...
                # max_FP = 0
                # relay = self.drone
                # for hello_pkt, drone in opt_neighbors:
                #     FP = util.projection_on_line_between_points(self.drone.coords, self.drone.depot.coords, hello_pkt.cur_pos)
                #     if FP > max_FP:
